...
class EDOExporter {
        ...
        private set(value) {
            ...
            value?.registerComponentCallbacks(object : ComponentCallbacks {
                override fun onLowMemory() {
                    this@EDOExporter.activeContexts.filter { !it.isReleased }.forEach { it.lowMemoryNotification() }
                }
    ...
    private val activeContexts: MutableList<V8> = mutableListOf()
    ...
    fun scriptObjectsWithObject(anObject: Any): List<V8Value> {
        return this.activeContexts.filter { !it.isReleased }.map {
